...
function openWindow(url, width, height, target) {
    ...
    var top = (cH - h) / 2 + window.screenTop;
    var left = (cW - w) / 2 + window.screenLeft;
    var features = "popup=yes, left=".concat(left, ", top=").concat(top, ", width=").concat(w, ", height=").concat(h);
...
function _getClient() {
    _getClient = _async_to_generator(function() {
        var response, ip;
...
function getComponentById(id, init) {
    ...
    if (allComponentMap.has(id)) return allComponentMap.get(id);
    else return void 0;
}
