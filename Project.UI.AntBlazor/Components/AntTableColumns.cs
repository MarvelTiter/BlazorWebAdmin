...
namespace Project.UI.AntBlazor.Components
{
    public class AntTableColumns<TData, TQuery> : ComponentBase where TQuery : IRequest, new()
    {
        ...
        private void BuildPropertyColumnDictionary(RenderTreeBuilder builder, ColumnInfo item)
        {
            ...
            builder.AddComponentParameter(1, "Property", (Expression<Func<Dictionary<string, object>, object>>)(c => c[item.PropertyOrFieldName]));
        ...
        private void BuildPropertyColumnDataTable(RenderTreeBuilder builder, ColumnInfo item)
        {
            ...
            builder.AddComponentParameter(1, "Property", (Expression<Func<DataRow, object>>)(c => c[item.PropertyOrFieldName]));
            AddColumnParameters(builder, item);
            builder.CloseComponent();
        }
        ...
        {
            builder.AddComponentParameter(2, "Ellipsis", col.Ellipsis);
            if (col.Width != null)
                builder.AddComponentParameter(3, "Width", col.Width);
            builder.AddComponentParameter(4, "Sortable", col.Sortable);
            builder.AddComponentParameter(5, "Title", RenderColumnTitle(col.Label));
            if (col.Fixed != null)
                builder.AddComponentParameter(6, "Fixed", col.Fixed);

            var fragment = cellrender.MakeGenericMethod(col.DataType).Invoke(null, [col]);

            builder.AddComponentParameter(7, "CellRender", fragment);
        }
        ...
        public static RenderFragment<CellData<T>> ColumnRender<T>(ColumnInfo col)
        {
            return context => builder =>
            {
                ...
                if (col.IsEnum || col.UseTag)
                {
                    ...
                    builder.AddComponentParameter(1, nameof(Tag.Color), color);
                    builder.AddComponentParameter(2, nameof(Tag.ChildContent), context.FormattedValue.AsContent());
                    builder.CloseComponent();
                }
                else if ((col.UnderlyingType ?? col.DataType) == typeof(bool))
                {
                    builder.OpenComponent<Switch>(0);
                    builder.AddComponentParameter(1, nameof(Switch.Disabled), true);
                    builder.AddComponentParameter(2, nameof(Switch.Checked), context.FieldValue);
                    builder.CloseComponent();
                }
                else
                {
